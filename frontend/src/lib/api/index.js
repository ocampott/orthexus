...
export const productosApi = {
  ...
  porBarcode:      (codigo)      => get(`/productos/barcode/${encodeURIComponent(codigo)}`),
  ...
  ajustarStock:    (id, cant, op='set') => patch(`/productos/${id}/stock`, { cantidad: cant, operacion: op }),
...
export const variantesApi = {
  ...
  editar:  (productoId, id, data) => put(`/productos/${productoId}/variantes/${id}`, data),
  ajustarStock: (productoId, id, cant, op = 'set') =>
                  patch(`/productos/${productoId}/variantes/${id}/stock`, { cantidad: cant, operacion: op }),
...
export const proveedoresApi = {
  ...
  subirLista:   async (id, archivo) => {
    ...
    const res = await fetch(`http://localhost:3001/api/proveedores/${id}/lista`, { method: 'POST', body: fd });
    const data = await res.json().catch(() => ({}));
...
export const uploadsApi = {
  subir: async (tipo, archivo) => {
    ...
    const res = await fetch(`http://localhost:3001/api/uploads/${tipo}`, { method: 'POST', body: fd });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || 'Error al subir archivo');
...
export const alquileresApi = {
  ...
  editarProducto:   (id, data)  => put(`/alquileres/productos-catalogo/${id}`, data),
